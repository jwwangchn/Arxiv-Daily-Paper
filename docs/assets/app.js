(function () {
  ...
  function buildAgentPrompt(paper) {
    return [
      ...
      "3. 从源码中抽取并理解论文正文、方法、实验、消融、图表和局限，而不是只依赖摘要。",
      "4. 生成一份详细中文 Markdown 报告，建议文件名为 report.md。",
      "5. 生成同内容的 HTML 报告，建议文件名为 report.html。HTML 应该可直接打开阅读，结构清晰，包含标题、目录、分节和表格。",
      "",
      "报告至少包含：",
      ...
      "- 图表解读：重要 figure/table 的作用、含义和对应结论。",
      "- 实验设置：数据集、baseline、指标、实现细节。",
      "- 主要结果与消融结论。",
      "- 创新点、局限性、可复现风险和后续研究建议。",
      "- 附录：你实际读取到的关键源码文件列表和图像文件列表。",
      "",
      "执行时请创建一个干净的工作目录，保留下载的源码和生成的两个报告。遇到源码缺失、编译不完整或图像格式特殊时，请在报告中说明。",
      "",
      "论文信息：",
      paperContext(paper),
