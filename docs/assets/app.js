(function () {
  ...
  function paperCard(paper) {
    ...
    return `<article class="paper-card"><h3 class="paper-title"><a href="${escapeHtml(paper.entry_url)}" target="_blank" rel="noopener">${escapeHtml(paper.title)}</a></h3><div class="paper-meta-line">${metaHtml}</div><div class="paper-authors">${escapeHtml(authors)}${more}</div><div class="paper-links"><a href="${escapeHtml(paper.entry_url)}" target="_blank" rel="noopener">arXiv</a><a href="${escapeHtml(paper.pdf_url)}" target="_blank" rel="noopener">PDF</a><span>${escapeHtml(paper.display_category || paper.primary_category)}</span></div>${analysisHtml}</article>`;
  }
